...
class Flocculator:
    ...
    def channel_W(self):
        ...
        channel_W = np.amax(np.array([1, (self.W_MIN/channel_est_W).to(u.dimensionless),
...
import numpy as np
Q=2 * u.L/u.s
myF = Flocculator(Q=Q, ent_tank_L=0 * u.m,W_MIN = 10*u.cm, max_L = 150*u.cm, downstream_H = 2 * u.m  )
...
channel_L = myF.channel_L * myF.channel_n/channel_n
channel_L
Tank_W = myF.channel_W*channel_n
Tank_W

Volume_floc =(myF.channel_n * myF.channel_W * myF.channel_L *myF.downstream_H).to(u.L)
Volume_floc
Volume_floc = (channel_L*Tank_W*myF.downstream_H).to(u.L)
Volume_floc
# 8 channels 30 cm wide, 50 cm long
